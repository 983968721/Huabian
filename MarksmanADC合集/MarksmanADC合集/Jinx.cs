...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("SwapQ" + Id, "自动 切换 Q").SetValue(false));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapDistance" + Id, "根据距离 切换Q").SetValue(true));
            config.AddItem(new MenuItem("SwapAOE" + Id, "打AOE伤害 切换Q").SetValue(false));
            config.AddItem(new MenuItem("MinWRange" + Id, "最小 W 范围").SetValue(new Slider(525 + 65 * 2, 0, 1200)));
            config.AddItem(new MenuItem("AutoEI" + Id, "自动-E (固定)").SetValue(true));
            config.AddItem(new MenuItem("AutoES" + Id, "自动-E (减速)").SetValue(true));
            config.AddItem(new MenuItem("AutoED" + Id, "自动-E (加速)").SetValue(false));
            config.AddItem(
                new MenuItem("CastR" + Id, "释放 R (2000 范围)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("ROverKill" + Id, "检查 R 击杀伤害").SetValue(true));
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "显示 Q 束缚").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 0))));
            config.AddItem(
                new MenuItem("DrawW" + Id, "W 范围").SetValue(new Circle(false, Color.CornflowerBlue)));
            return true;
        }
