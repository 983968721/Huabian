...
namespace Marksman
{
    ...
    internal class Kalista : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("JumpTo" + Id, "跳到").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawConnMax" + Id, "绑定 范围").SetValue(new Circle(false,
                    Color.FromArgb(100, 255, 255, 255))));
            config.AddItem(
                new MenuItem("DrawConnText" + Id, "绑定 文本").SetValue(new Circle(false, Color.GreenYellow)));
            config.AddItem(
                new MenuItem("DrawConnSignal" + Id, "绑定 信号").SetValue(true));
            config.AddItem(new MenuItem("Dx" + Id, ""));
            config.AddItem(
                new MenuItem("DrawEStackCount" + Id, "E 堆叠层数").SetValue(new Circle(false, Color.Firebrick)));
            config.AddItem(new MenuItem("Dx", ""));
            config.AddItem(
                new MenuItem("DrawJumpPos" + Id, "无伤打野").SetValue(new Circle(false, Color.HotPink)));

            var damageAfterE = new MenuItem("DamageAfterE", "打完伤害后 E").SetValue(true);
