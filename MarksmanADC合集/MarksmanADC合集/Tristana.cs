...
namespace Marksman
{
    internal class Tristana : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("UseRM" + Id, "使用 R 丨 击杀").SetValue(true));
            config.AddItem(new MenuItem("UseRMG" + Id, "使用 R 丨 防突进").SetValue(true));
            config.AddItem(new MenuItem("UseRMI" + Id, "使用 R 丨 打断法术").SetValue(true));
            return true;
        }
