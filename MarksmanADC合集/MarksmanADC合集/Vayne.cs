...
namespace Marksman
{
    internal class Vayne : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("UseET" + Id, "使用 E (自动)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseEInterrupt" + Id, "使用 E 打断").SetValue(true));
            config.AddItem(
                new MenuItem("PushDistance" + Id, "E 推的地方").SetValue(new Slider(425, 475, 300)));
            config.AddItem(new MenuItem("CompleteSilverBuff" + Id, "Q 激活 被动").SetValue(true));
            return true;
        }
