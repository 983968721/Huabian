...
namespace Marksman
{
    internal class Varus : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("UseQC" + Id, "Q").SetValue(new StringList(new[] {"关闭", "任何时刻", "W 堆叠时", "最大 W 层数"}, 3)));
            config.AddItem(new MenuItem("UseQMinChargeC" + Id, "最小 Q 充能").SetValue(new Slider(1000, 250, 1400)));
        ...
        public override bool HarassMenu(Menu config)
        {
            config.AddItem(
                new MenuItem("UseQH" + Id, "Q").SetValue(
                    new StringList(new[] {"关闭", "任何时刻", "W 堆叠时", "最大 W 层数"}, 3)));
        ...
        {
            config.AddItem(new MenuItem("spellDelay" + Id, "技能 延迟").SetValue(new Slider(500, 0, 3000)));
            config.AddItem(
                new MenuItem("CastR" + Id, "释放 R").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("DrawQC" + Id, "最小 Q 充能").SetValue(new Circle(true, Color.White)));
            config.AddItem(new MenuItem("DrawRS" + Id, "R: 搜索 范围").SetValue(new Circle(true, Color.White)));
