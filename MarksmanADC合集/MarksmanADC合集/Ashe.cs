...
namespace Marksman
{
    internal class Ashe : Champion
    {
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseWTH" + Id, "使用 W (自动)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            return true;
        }
        ...
        {
            config.AddItem(new MenuItem("RInterruptable" + Id, "自动R 打断法术").SetValue(true));
            config.AddItem(new MenuItem("EFlash" + Id, "使用E 查看闪现位置").SetValue(true));
