...
namespace EasyJinx
{
    class EasyKalista : Champion
    {
        ...
        protected override void InitializeMenu()
        {
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("Combo_erange", "敌人离开攻击范围 使用 E").SetValue(true));
            Menu.SubMenu("Combo").AddItem(new MenuItem("Combo_e", "使用 E # 堆叠").SetValue(false));
            Menu.SubMenu("Combo").AddItem(new MenuItem("Combo_estacks", "# 堆叠").SetValue(new Slider(4, 1, 10)));

            Menu.AddSubMenu(new Menu("骚 扰", "Harass"));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_q", "使用 Q").SetValue(true));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_e", "使用 E on # 堆叠").SetValue(true));
            Menu.SubMenu("Harass").AddItem(new MenuItem("Harass_estacks", "# 堆叠").SetValue(new Slider(4, 1, 10)));
            
            Menu.AddSubMenu(new Menu("自 动", "Auto"));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_q", "使用 Q").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_ekill", "使用 E 击杀").SetValue(true));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_e", "使用 E 堆叠").SetValue(false));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_estacks", "# 堆叠").SetValue(new Slider(4, 1, 10)));
            Menu.SubMenu("Auto").AddItem(new MenuItem("Auto_ejungle", "使用 E 防止gank").SetValue(true));
            
            Menu.AddSubMenu(new Menu("范 围", "Drawing"));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_q", "Q 范 围").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_e", "E 范 围").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_r", "R 范 围").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 0, 255, 0))));
            Menu.SubMenu("Drawing").AddItem(new MenuItem("Drawing_edamage", "E 伤害指示器").SetValue(true));
        }
