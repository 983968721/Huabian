...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("花边汉化-Esk0r卡牌", "TwistedFate", true);

            var TargetSelectorMenu = new Menu("目标 杂项", "Target Selector");
            TargetSelector.AddToMenu(TargetSelectorMenu);
            Config.AddSubMenu(TargetSelectorMenu);

            var SowMenu = new Menu("走 砍", "Orbwalking");
            SOW = new Orbwalking.Orbwalker(SowMenu);
            Config.AddSubMenu(SowMenu);

            /* Q */
            var q = new Menu("Q - 配置", "Q");
            q.AddItem(new MenuItem("AutoQI", "自动-Q(目标被眩晕)").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "自动-Q(目标在移动)").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "自动-Q (手动)").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);

            /* W */
            var w = new Menu("W - 选牌", "W");
            w.AddItem(
                new MenuItem("SelectYellow", "黄牌 按键").SetValue(new KeyBind("W".ToCharArray()[0],
                    KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectBlue", "蓝牌 按键").SetValue(new KeyBind("E".ToCharArray()[0], KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectRed", "红牌 按键").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(w);

            var r = new Menu("R - 设置", "R");
            r.AddItem(new MenuItem("AutoY", "落地 自动黄牌").SetValue(true));
            Config.AddSubMenu(r);

            var misc = new Menu("杂 项", "Misc");
            misc.AddItem(new MenuItem("PingLH", "敌人残血自动R (国服还是不可用)").SetValue(true));
            ...
            Config.AddItem(new MenuItem("Combo", "连招 按键").SetValue(new KeyBind(32, KeyBindType.Press)));
