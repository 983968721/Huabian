...
The idea where the lines come from is that u can calculate how far they are from base (enemySpawnPos) , so is it possible to make a script that can just show the position of the enemy while recalling i guess it would help , especially if u can show these lines even when they're not recalling (not sure if it's possible tho ) that would help so much in ganks and other stuff , thanks for your time , and i hope you give me your opinion about this script , have a nice day 
    ...
    internal class BaseUlt
    {
        ...
        public BaseUlt()
        {
            (Menu = new Menu("花边汉化-基地大招3", "BaseUlt3", true)).AddToMainMenu();
            Menu.AddItem(new MenuItem("showRecalls", "显示 回城").SetValue(true));
            Menu.AddItem(new MenuItem("baseUlt", "全图 大招").SetValue(true));
            Menu.AddItem(new MenuItem("checkCollision", "检查 碰撞").SetValue(true));
            Menu.AddItem(new MenuItem("panicKey", "禁用 大招 SBTW").SetValue(new KeyBind(32, KeyBindType.Press))); //32 == space
            Menu.AddItem(new MenuItem("regardlessKey", "关闭 时间限制 (保持)").SetValue(new KeyBind(17, KeyBindType.Press))); //17 == ctrl
            ...
            TeamUlt = Menu.AddSubMenu(new Menu("大招 支援队友", "TeamUlt"));
            DisabledChampions = Menu.AddSubMenu(new Menu("禁用 大招支援", "DisabledChampions"));

            if (compatibleChamp)
            {
                foreach (Obj_AI_Hero champ in Allies.Where(x => !x.IsMe && IsCompatibleChamp(x.ChampionName)))
                    TeamUlt.AddItem(new MenuItem(champ.ChampionName, "支援 友军: " + champ.ChampionName).SetValue(false).DontSave());

                foreach (Obj_AI_Hero champ in Enemies)
                    DisabledChampions.AddItem(new MenuItem(champ.ChampionName, "设置不对释放大招目标: " + champ.ChampionName).SetValue(false).DontSave());
            }
            ...
            Game.PrintChat("<font color=\"#1eff00\">BaseUlt3 by Beaving -Loaded</font> - <font color=\"#00BFFF\">花边-汉化!</font>");
        }
