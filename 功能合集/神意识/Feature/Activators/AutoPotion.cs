		...
		private void ActivatorsLoaded(object o)
		{
			try
			{
				if (o is Activators && (o as Activators).Menu != null)
				{
					...
					var healthMenu = new Menu("H P", Name + "Health");
					healthMenu.AddItem(new MenuItem(Name + "HealthPotion", "使用 红色药剂").SetValue(true));
					healthMenu.AddItem(
						new MenuItem(Name + "HealthPercent", "触发百分比").SetValue(new Slider(60)));

					var manaMenu = new Menu("M P", Name + "Mana");
					manaMenu.AddItem(new MenuItem(Name + "ManaPotion", "使用 蓝色药剂").SetValue(true));
					manaMenu.AddItem(new MenuItem(Name + "ManaPercent", "触发百分比").SetValue(new Slider(60)));

					Menu.AddSubMenu(healthMenu);
					Menu.AddSubMenu(manaMenu);

					Menu.AddItem(new MenuItem(Name + "Enabled", "开 启").SetValue(false));
