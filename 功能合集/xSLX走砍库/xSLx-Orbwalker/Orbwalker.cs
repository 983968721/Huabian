...
namespace xSLx_Orbwalker
{
	...
	public class xSLxOrbwalker
	{
		...
		public static void AddToMenu(Menu menu)
		{
			...
			var menuDrawing = new Menu("范 围", "orb_Draw");
			menuDrawing.AddItem(new MenuItem("orb_Draw_AARange", "平A 范围").SetValue(new Circle(true, Color.FloralWhite)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_AARange_Enemy", "敌人 平A 距离").SetValue(new Circle(true, Color.Pink)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_Holdzone", "控制 区域").SetValue(new Circle(true, Color.FloralWhite)));
			...
			menuDrawing.AddItem(new MenuItem("orb_Draw_hitbox", "显示 打击模型").SetValue(new Circle(true, Color.FloralWhite)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_Lasthit", "显示 补刀血量").SetValue(new Circle(true, Color.Lime)));
			menuDrawing.AddItem(new MenuItem("orb_Draw_nearKill", "显示 附近可击杀 小兵").SetValue(new Circle(true, Color.Gold)));
			menu.AddSubMenu(menuDrawing);

			var menuMisc = new Menu("杂 项", "orb_Misc");
			menuMisc.AddItem(new MenuItem("orb_Misc_Holdzone", "保持 位置").SetValue(new Slider(50, 100, 0)));
			menuMisc.AddItem(new MenuItem("orb_Misc_Farmdelay", "打钱 延迟").SetValue(new Slider(0, 200, 0)));
			menuMisc.AddItem(new MenuItem("orb_Misc_ExtraWindUp", "额外 结束时间").SetValue(new Slider(80, 200, 0)));
			menuMisc.AddItem(new MenuItem("orb_Misc_AutoWindUp", "自动设置 结束").SetValue(false));
			menuMisc.AddItem(new MenuItem("orb_Misc_Priority_Unit", "优先 单位").SetValue(new StringList(new[] { "小兵", "英雄" })));
			menuMisc.AddItem(new MenuItem("orb_Misc_Humanizer", "移动 延迟").SetValue(new Slider(80, 200, 15)));
			menuMisc.AddItem(new MenuItem("orb_Misc_AllMovementDisabled", "禁止 所有移动").SetValue(false));
			menuMisc.AddItem(new MenuItem("orb_Misc_AllAttackDisabled", "禁止 所有攻击").SetValue(false));

			menu.AddSubMenu(menuMisc);

			var menuMelee = new Menu("近战", "orb_Melee");
			menuMelee.AddItem(new MenuItem("orb_Melee_Prediction", "移动 预测").SetValue(false));
			...
			menu.AddItem(new MenuItem("xSLx_info", "版权: xSLx"));
			menu.AddItem(new MenuItem("xSLx_info2", "制作者: xSLx & Esk0r"));
			menu.AddItem(new MenuItem("xSLx_info3", "花边汉化-sXLx走砍"));
