...
namespace KaiHelper.Timer
{
    public class JungleTimer
    {
        ...
        {
            _menuJungle = config.AddSubMenu(new Menu("打野 计时", "JungleTimer"));
            _menuJungle.AddItem(
                new MenuItem("JungleTimerFormat", "时间 格式:").SetValue(new StringList(new[] { "分/秒", "秒" })));
            _menuJungle.AddItem(new MenuItem("JungleActive", "开 启!").SetValue(true));
            _jungleCamps.Add(
                new JungleCamp(
