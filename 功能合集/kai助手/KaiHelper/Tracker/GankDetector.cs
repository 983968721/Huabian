...
namespace KaiHelper.Tracker
{
    ...
    public class GankDetector
    {
        ...
        {
            _menuGank = config.AddSubMenu(new Menu("Gank 警报", "GDetect"));
            _menuGank.AddItem(new MenuItem("InvisibleTime", "消失 时间").SetValue(new Slider(5, 1, 10)));
            _menuGank.AddItem(new MenuItem("VisibleTime", "可见 时间").SetValue(new Slider(3, 1, 5)));
            _menuGank.AddItem(new MenuItem("TriggerRange", "触发 范围").SetValue(new Slider(3000, 1, 3000)));
            _menuGank.AddItem(new MenuItem("CircalRange", "追踪 范围").SetValue(new Slider(2500, 1, 3000)));
            _menuGank.AddItem(new MenuItem("GankType", "类型:").SetValue(new StringList(new []{"线条","线圈","都有"})));
            _menuGank.AddItem(new MenuItem("Ping", "聊天窗口 警报").SetValue(true));
            _menuGank.AddItem(new MenuItem("GankActive", "开 启").SetValue(true));
