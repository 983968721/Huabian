...
namespace VallySKO_Gnar
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            SKOMenu = new Menu("花边汉化-SKO纳尔", "VSGnar", true);

            var SKOTs = new Menu("目标 选择", "TargetSelector");
            TargetSelector.AddToMenu(SKOTs);

            SKOMenu.AddSubMenu(new Menu("走 砍", "Orbwalking"));
			...
			Combo.AddItem(new MenuItem("UseEunder", "目标在塔下丨自动E").SetValue(true));
            ...
            Combo.AddItem(new MenuItem("activeCombo", "开 启!").SetValue(new KeyBind(32, KeyBindType.Press)));
            ...
            JLClear.AddItem(new MenuItem("activeClear", "开 启!").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));

            var KillSteal = new Menu("击 杀", "KillSteal");
            KillSteal.AddItem(new MenuItem("Foguinho", "使用 点燃").SetValue(true));
            KillSteal.AddItem(new MenuItem("UseQKs", "使用 Q").SetValue(true));
            KillSteal.AddItem(new MenuItem("UseWKs", "使用 W").SetValue(true));

            var Drawc = new Menu("范 围", "Drawing");
            Drawc.AddItem(new MenuItem("DrawQ", "范 围 Q").SetValue(true));
            Drawc.AddItem(new MenuItem("DrawW", "范 围 W").SetValue(true));
            Drawc.AddItem(new MenuItem("DrawE", "范 围 E").SetValue(true));
            Drawc.AddItem(new MenuItem("DrawR", "范 围 R").SetValue(true));
            Drawc.AddItem(new MenuItem("CircleLag", "线圈 长度").SetValue(true));
            Drawc.AddItem(new MenuItem("CircleQuality", "线圈 宽度").SetValue(new Slider(100, 100, 10)));
            Drawc.AddItem(new MenuItem("CircleThickness", "线圈 厚度").SetValue(new Slider(1, 10, 1)));
            ...
            Game.PrintChat("<font color='#07B88C'>花边汉化-SKO纳尔 加载成功!</font>");
