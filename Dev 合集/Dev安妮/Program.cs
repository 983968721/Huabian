    ...
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            try
            {
                ...
                Game.PrintChat(string.Format("<font color='#fb762d'>DevAnnie Loaded v{0}汉化by花边</font>", Assembly.GetExecutingAssembly().GetName().Version));
        ...
        {
            Config = new Menu("花边汉化-DEV安妮", "DevAnnie", true);
            ...
            Config.SubMenu("Mode").AddItem(new MenuItem("ModeType", "模式切换:").SetValue(new StringList(new[] { "单人路", "辅助" })));

            Config.AddSubMenu(new Menu("被动 堆叠", "Passive"));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseEStackPassive", "使用E 堆叠(下面是E使用选项)").SetValue(true));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseEStackPassiveNoEnemy", "安全时 总是堆叠").SetValue(true));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseEStackPassiveFountain", "在泉水时丨堆叠").SetValue(true));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseEStackPassiveEverywhere", "任何情况丨都堆叠").SetValue(false));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseQStackPassive", "使用Q 堆叠(下面是Q使用选项)").SetValue(true));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseQStackPassiveLastHit", "仅 最后一下").SetValue(true));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseQStackPassiveNoEnemy", "安全时丨总是堆叠").SetValue(true));
            Config.SubMenu("Passive").AddItem(new MenuItem("UseQStackPassiveEverywhere", "任何情况丨都堆叠").SetValue(false));

            Config.AddSubMenu(new Menu("闪现 连招", "FlashCombo"));
            Config.SubMenu("FlashCombo").AddItem(new MenuItem("FlashComboKey", "开 启!").SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
            Config.SubMenu("FlashCombo").AddItem(new MenuItem("FlashComboMinEnemies", "闪现连招丨敌人数").SetValue(new Slider(2, 1, 5)));
            Config.SubMenu("FlashCombo").AddItem(new MenuItem("FlashAntiSuicide", "使用闪现丨防止自杀").SetValue(true));
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRMinEnemies", "使用R眩晕丨敌人数").SetValue(new Slider(2, 1, 5)));

            Config.AddSubMenu(new Menu("骚 扰", "Harass"));
            Config.SubMenu("Harass").AddItem(new MenuItem("UseQHarass", "使用 Q").SetValue(true));
            Config.SubMenu("Harass").AddItem(new MenuItem("UseQHarassLastHit", "使用Q最后一下丨堆叠眩晕").SetValue(true));
            ...
            Config.SubMenu("LaneClear").AddItem(new MenuItem("ManaLaneClear", "使用W清线丨最低蓝量").SetValue(new Slider(30, 1, 100)));

            Config.AddSubMenu(new Menu("清 野", "JungleClear"));
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("UseEAgainstAA", "使用 E 之前平A一下").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("UseRInterrupt", "使用 R + 放火 打断").SetValue(true));

            Config.AddSubMenu(new Menu("防 突进", "GapCloser"));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("UseEGapCloser", "被突进时丨使用E").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloser", "被突进时丨使用屏障").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloserMinHealth", "使用屏障丨最低血量").SetValue(new Slider(40, 0, 100)));
            ...
            Config.SubMenu("Drawings").AddItem(new MenuItem("ComboDamage", "显示 血条").SetValue(true));
