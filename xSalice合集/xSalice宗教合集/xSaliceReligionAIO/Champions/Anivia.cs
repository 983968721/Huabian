    ...
    class Anivia : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("键位", "Key");
            {
                ...
                key.AddItem(new MenuItem("LaneClearActive", "清闲!",true).SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
            ...
            var spellMenu = new Menu("技能 菜单", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("qHit", "连招 命中率",true).SetValue(new Slider(3, 1, 3)));
                    qMenu.AddItem(new MenuItem("qHit2", "骚扰 命中率",true).SetValue(new Slider(3, 1, 4)));
                    qMenu.AddItem(new MenuItem("detonateQ", "自动 Q",true).SetValue(true));
                    qMenu.AddItem(new MenuItem("detonateQ2", "自动 Q 落后于敌人",true).SetValue(true));
                    spellMenu.AddSubMenu(qMenu);
                }

                var wMenu = new Menu("W 技能", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("wallKill", "可击杀使用W",true).SetValue(true));
                    spellMenu.AddSubMenu(wMenu);
                }

                var rMenu = new Menu("R 技能", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("checkR", "自动 R",true).SetValue(true));
            ...
            {
                misc.AddItem(new MenuItem("UseInt", "使用技能打断技能",true).SetValue(true));
                misc.AddItem(new MenuItem("UseGap", "使用 W 防止突击",true).SetValue(true));
