    ...
    class Lucian : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("Botrk", "使用 破败/小弯刀", true).SetValue(true));
                //add to menu
            ...
            {
                misc.AddItem(new MenuItem("CheckPassive", "智能 刷被动", true).SetValue(true));
                misc.AddItem(new MenuItem("smartKS", "使用 智能抢人头", true).SetValue(true));
                misc.AddItem(new MenuItem("E_If_HP", "禁用E|自己血量", true).SetValue(new Slider(20)));
                //add to menu
