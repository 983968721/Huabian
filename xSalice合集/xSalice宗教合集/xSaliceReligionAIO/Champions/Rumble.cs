    ...
    class Rumble : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("键位", "Key");
            {
                ...
                key.AddItem(new MenuItem("LastHitE", "使用 E 补刀!", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("UseMecR", "强制使用 R ", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能菜单", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Auto_Heat", "使用 Q （过热）", true).SetValue(true));
                    qMenu.AddItem(new MenuItem("Q_Over_Heat", "智能Q抢人头", true).SetValue(true));
                    spellMenu.AddSubMenu(qMenu);
                }

                var wMenu = new Menu("W 技能", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("W_Auto_Heat", "使用 W （过热）", true).SetValue(true));
                    wMenu.AddItem(new MenuItem("W_Always", "连招|骚扰总是使用W", true).SetValue(false));
                    wMenu.AddItem(new MenuItem("W_Block_Spell", "连招优先使用W", true).SetValue(true));
                    spellMenu.AddSubMenu(wMenu);
                }

                var eMenu = new Menu("E 技能", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Auto_Heat", "使用 E （过热）", true).SetValue(false));
                    eMenu.AddItem(new MenuItem("E_Over_Heat", "智能E抢人头", true).SetValue(true));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R 技能", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_If_Enemy_Count", "自动R|敌人数量", true).SetValue(new Slider(4, 1, 6)));
                    rMenu.AddItem(new MenuItem("R_If_Enemy_Count_Combo", "连招使用R|敌人数量", true).SetValue(new Slider(3, 1, 6)));
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("qHit", "E 命中率", true).SetValue(new Slider(3, 1, 3)));
            ...
            var harass = new Menu("骚扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("qHit2", "E 命中率", true).SetValue(new Slider(3, 1, 4)));
                //add to menu
            ...
            var drawMenu = new Menu("范围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_R_Pred", "显示 R最佳路径", true).SetValue(true));
