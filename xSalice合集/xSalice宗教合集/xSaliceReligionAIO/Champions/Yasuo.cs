    ...
    class Yasuo : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("键位", "Key");
            {
                ...
                key.AddItem(new MenuItem("Flee", "使用 E 逃避", true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能 菜单", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Auto", "自动 Q 目标", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
                    qMenu.AddItem(new MenuItem("Q_Auto_third", "自动 Q3 击飞", true).SetValue(true));
                    qMenu.AddItem(new MenuItem("Q_UnderTower", "塔下 自动 Q", true).SetValue(false));
                    qMenu.AddItem(new MenuItem("Q_Stack", "手动 Q3 目标", true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Toggle)));
                    qMenu.AddItem(new MenuItem("Q_thirdE", "优先 E->3rd Q -> 单一 Q", true).SetValue(true));
                ...
                {
                    eMenu.AddItem(new MenuItem("E_Min_Dist", "使用 E 最小距离", true).SetValue(new Slider(250, 1, 475)));
                    //e Evade
                    ...
                    eMenu.AddItem(new MenuItem("E_GapClose", "使用 E 突进", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Turret", "不要 E 进炮塔", true).SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Toggle)));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R 技能", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_If_Killable", "可击杀使用R", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("delayR", "延迟 用R 击杀敌人", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("R_MEC", "自动R>= 敌人数量, 0 = 关闭", true).SetValue(new Slider(3, 0, 5)));
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("qHit", "Q 命中率", true).SetValue(new Slider(2, 1, 3)));
                combo.AddItem(new MenuItem("UseECombo", "使用 E", true).SetValue(true));
                combo.AddItem(new MenuItem("UseRCombo", "使用 R", true).SetValue(true));
                combo.AddItem(new MenuItem("ComboR_MEC", "使用 R 丨敌人数量", true).SetValue(new Slider(3, 1, 5)));
                //add to menu
            ...
            var harass = new Menu("骚扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("qHit2", "Q 命中率", true).SetValue(new Slider(2, 1, 3)));
            ...
            var farm = new Menu("清线", "Farming");
            {
                ...
                farm.AddItem(new MenuItem("UseQFarm", "使用Q 清线", true).SetValue(true));
                ...
                farm.AddItem(new MenuItem("E_UnderTower_Farm", "塔下使用E清线", true).SetValue(false));
                farm.AddItem(new MenuItem("LaneClear_useQ_minHit", "使用Q小兵数量", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            var misc = new Menu("杂项", "Misc");
            {
                ...
                misc.AddItem(new MenuItem("Interrupt", "打断技能", true).SetValue(true));
                misc.AddItem(new MenuItem("predMode", "预测模式  关闭 = 迅速, 开启 = 精确", true).SetValue(true));
            ...
            var drawMenu = new Menu("范围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_Q2", "范围 Q 延伸", true).SetValue(true));
