    ...
    class Zed : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("键位", "Key");
            {
                ...
                key.AddItem(new MenuItem("LastHitQ", "补刀 Q!", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("Switch_1", "清线 模式", true).SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("Switch_2", "哄骗 模式", true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("Escape", "W 向鼠标方向逃跑", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能 菜单", "SpellMenu");
            {
                ...
                {
                    wMenu.AddItem(new MenuItem("W_Require_QE", "W骚扰到敌方同时使用QE", true).SetValue(true));
                    wMenu.AddItem(new MenuItem("W_Follow_Combo", "先手W连招", true).SetValue(false));
                    wMenu.AddItem(new MenuItem("useW_Health", "血量低连招后使用W逃跑", true).SetValue(new Slider(25)));
                    spellMenu.AddSubMenu(wMenu);
                }

                var rMenu = new Menu("R 技能", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_Place_line", "使用R|目标进入范围", true).SetValue(new Slider(400, 250, 550)));
                    rMenu.AddItem(new MenuItem("R_Back", "R杀死敌人自动返回", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("useR_Health", "血量低大招后返回", true).SetValue(new Slider(10)));
            ...
            {
                combo.AddItem(new MenuItem("selected", "攻击 选择目标", true).SetValue(true));
                combo.AddItem(new MenuItem("Combo_mode", "连招 模式", true).SetValue(new StringList(new[] { "Normal", "Line Combo", "Coax", "Ult no W", "Normal With Ult" })));
                combo.AddItem(new MenuItem("Combo_Switch", "开关 模式", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
                combo.AddItem(new MenuItem("UseQCombo", "使用 Q", true).SetValue(true));
                combo.AddItem(new MenuItem("Prioritize_Q", "使用 W->Q", true).SetValue(true));
            ...
            var farm = new Menu("清线", "LaneClear");
            {
                ...
                farm.AddItem(new MenuItem("LaneClear_useE_minHit", "使用E|可攻击到敌人数量", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
                menu.AddSubMenu(farm);
            }
            var misc = new Menu("杂项", "Misc");
            {
                misc.AddItem(new MenuItem("smartKS", "使用 智能抢人头", true).SetValue(true));
                misc.AddItem(new MenuItem("Use_W_KS", "使用 W 击杀", true).SetValue(true));
                misc.AddItem(new MenuItem("AutoE", "敌人在范围内自动E", true).SetValue(true));
                //add to menu
