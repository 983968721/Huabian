...
namespace Support.Plugins
{
    ...
    public class Blitzcrank : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddSlider("ComboCountR", "对范围内敌人使用大招", 2, 1, 5);
        }

        public override void MiscMenu(Menu config)
        {
            config.AddBool("Misc.Q.Block", "用 Q 阻止敌人靠近", true);
            config.AddSlider("Misc.Q.Block.Distance", "Q 限制距离", 400, 0, 800);
        }

        public override void HarassMenu(Menu config)
        {
            config.AddBool("HarassQ", "使用 Q 骚扰", true);
        }

        public override void InterruptMenu(Menu config)
        {
            config.AddBool("GapcloserE", "使用 E 阻止 突进", true);
            config.AddBool("GapcloserR", "使用 R 阻止 突进", true);
            config.AddBool("InterruptQ", "使用 Q 打断 技能", true);
            config.AddBool("InterruptE", "使用 E 打断 技能", true);
            config.AddBool("InterruptR", "使用 R 打断 技能", true);
        }
    }
}